...
void Game::playTurn() {
    ...
    if ((this->p1).getPointer() == (this->p2).getPointer()) {
        throw std::runtime_error("This function threw an error!");
    }
...
void Game::printStats() {
    ...
    cout << "win rate: " << calculationPercentages(winRate(1),amountOfTurn())<< "%" << endl; 
    cout << "cards won: " << (this->p1).cardesTaken() << endl; 
    cout << "type cards won: " << (this->p1).cardesWin()  << endl; 
    cout << endl; 

    cout << "Stats " << (this->p2).getName() << ": "<< endl; 
    cout << "win rate: " <<calculationPercentages(winRate(2),amountOfTurn()) << "%" << endl; 
    cout << "cards won: " << (this->p2).cardesTaken() << endl; 
    cout << "type cards won: " << (this->p2).cardesWin()  << endl; 
    cout << endl; 
    cout << "amount of turn "<<amountOfTurn()<<endl;
    cout << "draw rate: " << calculationPercentages(amountOfDrawsTurn(),amountOfTurn())<< "%" << endl; 
    cout << "amount of draws: " << amountOfDraws() << endl;    
}
int Game::calculationPercentages(int part,int total){
    return (((double)part)/((double) total))*100;
}

int Game::amountOfDrawsTurn(){
    int count = 0;
    bool testDraw = false;
    for (int i=0; i<26; ++i){
        if(typeTurn[i] == 3){
